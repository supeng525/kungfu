...
#include "gatewayUFX/UFXGateway.h"
...
namespace kungfu
{
    namespace wingchun
    {
        namespace ufx
        {
            class TraderUFX : public UFX_Callback, public broker::Trader
            {
            public:
                TraderUFX(bool low_latency, yijinjing::data::locator_ptr locator, const std::string &account_id, const std::string &json_config);
				...
				void _onOrderEvent(std::shared_ptr<ufx_data::OrderInfo> &orderEvent);
				void _onTradeEvent(std::shared_ptr<ufx_data::TradeInfo> &tradeEvent);
				void _onQueryPosition(std::shared_ptr<ufx_data::PositionInfo> &pos, bool is_last);
				void _onQueryAsset(std::shared_ptr<ufx_data::AccountInfo> &acc);
				void msg(string m);
