...
#ifndef KUNGFU_UFX_EXT_MARKET_DATA_H
#define KUNGFU_UFX_EXT_MARKET_DATA_H
...
namespace kungfu
{
    namespace wingchun
    {
        namespace ufx
        {
            class MarketDataUFX : public MdCallback, public broker::MarketData
            {
            public:
                MarketDataUFX(bool low_latency, yijinjing::data::locator_ptr locator, const std::string &json_config);

                ~MarketDataUFX() override;
				...
				void onTick(int day, int time, const char* code, float open, float high, float low, float close);
				void OnDepthMarketData(const TDF_MARKET_DATA *tdfMarketData);
				void OnOrderByTick(const TDF_ORDER *tdfOrder);
				void OnTransByTick(const TDF_TRANSACTION *tdfTrans);
				void msg(string m);
